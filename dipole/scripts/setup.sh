...
spack load root@${ROOT_VERSION}; if [ $? -eq 0 ] ; then echo "root ${ROOT_VERSION} loaded: location - $(spack location -i root@${ROOT_VERSION})"; else echo "Failed to load root ${ROOT_VERSION}"; exit 1; fi
